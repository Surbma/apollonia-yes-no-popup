...
add_action( 'wp_footer', 'apollonia_yes_no_popups_show_popup_3' );
function apollonia_yes_no_popups_show_popup_3() {
	$options = get_option( 'apollonia_yes_no_popups_popup_3_fields' );

	if( $options['popupshoweverywhere'] == 1 ) {
		add_action( 'wp_footer', 'apollonia_yes_no_popups_popup_3_block', 999 );
	}
	else {
...
function apollonia_yes_no_popups_popup_3_block() {
	if( !isset( $_COOKIE['apollonia-yes-no-popups-popup-3'] ) || ( defined( 'WP_DEBUG' ) && WP_DEBUG == true ) ) {
		$options = get_option( 'apollonia_yes_no_popups_popup_3_fields' );
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$.UIkit.modal(('#apollonia-yes-no-popups-popup-3'), {center: true,keyboard: false,bgclose: false}).show();
		});
	</script>
	<div id="apollonia-yes-no-popups-popup-3" class="uk-modal">
	...
	<script type="text/javascript">
		function setCookie() {
		    ...
		    document.cookie = "apollonia-yes-no-popups-popup-3=yes;" + expires + ";path=/";
		}
