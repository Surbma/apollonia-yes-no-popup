...
add_action( 'wp_footer', 'apollonia_yes_no_popups_show_popup_2' );
function apollonia_yes_no_popups_show_popup_2() {
	$options = get_option( 'apollonia_yes_no_popups_popup_2_fields' );

	if( $options['popupshoweverywhere'] == 1 ) {
		add_action( 'wp_footer', 'apollonia_yes_no_popups_popup_2_block', 999 );
	}
	else {
...
function apollonia_yes_no_popups_popup_2_block() {
	if( !isset( $_COOKIE['apollonia-yes-no-popups-popup-2'] ) || ( defined( 'WP_DEBUG' ) && WP_DEBUG == true ) ) {
		$options = get_option( 'apollonia_yes_no_popups_popup_2_fields' );
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$.UIkit.modal(('#apollonia-yes-no-popups-popup-2'), {center: true,keyboard: false,bgclose: false}).show();
		});
	</script>
	<div id="apollonia-yes-no-popups-popup-2" class="uk-modal">
	...
	<script type="text/javascript">
		function setCookie() {
		    ...
		    document.cookie = "apollonia-yes-no-popups-popup-2=yes;" + expires + ";path=/";
		}
