...
/*
Plugin Name: Apollonia - Yes/No Popup
...
Text Domain: apollonia-yes-no-popup
...
define( 'APOLLONIA_YES_NO_POPUP_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
define( 'APOLLONIA_YES_NO_POPUP_PLUGIN_URL', plugins_url( '', __FILE__ ) );

// Localization
add_action( 'plugins_loaded', 'apollonia_yes_no_popup_init' );
function apollonia_yes_no_popup_init() {
	load_plugin_textdomain( 'apollonia-yes-no-popup', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
}

// Include files
if ( is_admin() ) {
	include_once( APOLLONIA_YES_NO_POPUP_PLUGIN_DIR . '/lib/admin.php' );
}

add_action( 'wp_enqueue_scripts', 'apollonia_yes_no_popup_enqueue_scripts', 999 );
function apollonia_yes_no_popup_enqueue_scripts() {
	wp_enqueue_script( 'apollonia-yes-no-popup-scripts', plugins_url( '', __FILE__ ) . '/js/scripts-min.js', array( 'jquery' ), '2.27.1', true );
	wp_enqueue_style( 'apollonia-yes-no-popup-styles', plugins_url( '', __FILE__ ) . '/css/styles.css', false, '2.27.1' );
}

add_action( 'wp_footer', 'apollonia_yes_no_popup_show' );
function apollonia_yes_no_popup_show() {
	$options = get_option( 'apollonia_yes_no_popup_fields' );

	if( $options['popupshoweverywhere'] == 1 ) {
		add_action( 'wp_footer', 'apollonia_yes_no_popup_block', 999 );
	}
	else {
...
function apollonia_yes_no_popup_block() {
	if( !isset( $_COOKIE['apollonia-yes-no-popup'] ) || ( defined( 'WP_DEBUG' ) && WP_DEBUG == true ) ) {
		$options = get_option( 'apollonia_yes_no_popup_fields' );
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$.UIkit.modal(('#apollonia-yes-no-popup'), {center: true,keyboard: false,bgclose: false}).show();
		});
	</script>
	<div id="apollonia-yes-no-popup" class="uk-modal">
	...
	<script type="text/javascript">
		function setCookie() {
		    ...
		    document.cookie = "apollonia-yes-no-popup=yes;" + expires + ";path=/";
		}
